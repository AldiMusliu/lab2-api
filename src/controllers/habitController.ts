...
export const createHabit = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    console.log('Creat habit error', e)
    res.status(500).json({
...
export const getUserHabits = async (
  ...
  try {
    ...
    const habitsWithTags = userHabitsWithTags.map((habit) => ({
      ...
      habitTags: undefined, // the habit that we are destructing have this habitTags, so to make sure to not include it because we are returning tags array,
      //we can do it this way by declarating the habitTags undefinded
    }))
